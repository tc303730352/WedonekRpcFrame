...
namespace SocketTcpClient.UpFile
{
        ...
        internal class UpFileTask : DataSyncClass
        {
                ...
                private readonly UpProgress _Progress = null;
                ...
                public UpFileTask(string direct, object param, FileInfo file, UpFileAsync async, string serverId, UpProgress progress, int timeOut, int syncTimeOut)
                {
                        this._UpParam = param;
                ...
                {
                        if (!this._LoadFile(out string error))
                        {
                                this.Error = error;
                                return false;
                        }
                        else if (!this._FileAccredit(out error))
                        {
                                this.Error = error;
                                return false;
                        }
                ...
                protected bool _LoadFile(out string error)
                {
                        try
                        {
                        ...
                        {
                                error = "socket.up.file.open.error";
                                new ErrorLog(e, "文件上传加载文件错误!", "Socket_Client")
                                {
                                        LogContent = this._File.FullName
                                }.Save();
                                return false;
                        }
                }
                ...
                private void _UpEnd(FileUpRes res)
                {
                        ...
                        FileUpResult result = new FileUpResult(res.UpResult)
                        {
                ...
                {
                        Interlocked.Exchange(ref this._AlreadyUpNum, state.AlreadyUpNum);
                ...
                private bool _FileAccredit(out string error)
                {
                        byte[] myByte = this._UpParam == null ? null : SocketTools.SerializationData(this._UpParam.GetType(), this._UpParam);
                        ...
                        FilePage data = new FilePage
                        {
                                ...
                                DirectName = _DirectName
                        };
                        Page page = Page.GetUpFilePage(this._ServerId, "FileAccredit", data, this._TimeOut, this._SyncTimeOut);
                        if (!PageManage.Send(page, out FileAccreditResult result, out error))
                        {
                                return false;
                        }
                ...
                private void _SendStream(int index)
                {
                        ...
                        if (this._Lock.GetLock())
                        {
                                ...
                                this._FileStream.Read(myByte, 6, len);
                                this._Lock.Exit();
                        }
                        Page page = Page.GetUpFileStream(this._ServerId, myByte, Config.SocketConfig.Timeout);
                ...
                public void CheckUpState(int sendTimeOut)
                {
                        ...
                        else if (this._UpProgress == UpFileProgress.连接断开 && ClientManage.Ping(this._ServerId, out _))
                        {
                                this._SyncUpState();
                        }
                }
                ...
                {
                        Page page = Page.GetUpFilePage(this._ServerId, "SyncUpState", this._PageId, this._TimeOut, this._SyncTimeOut);
