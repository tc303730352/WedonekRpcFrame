...
namespace WeDonekRpc.Client.TranService
{
    ...
    internal class RpcTranService : IRpcTranService
    {
        ...
        internal static ICurTranState SourceTran
        {
            get => _SourceTran.Value;
        }
        ...
        {
            get => RpcTranService._SourceTran.Value?.TranId;
        }
        ...
        internal static bool AddQueueTranLog (RemoteMsg msg)
        {
            ...
            if (RemoteCollect.Send(tran.RpcMerId, tran.RegionId, log, out long id, out string error))
            {
                ICurTranState cur = new TranStateVal(id, template, tran, msg.MsgBody);
                cur.BeginTran();
                RpcTranService._SourceTran.Value = new TranStateVal(tran);
                RpcTranService._CurTran.Value = cur;
                return true;
            }
            else if (error.StartsWith("rpc.tran."))
            {
                return false;
            }
            else
