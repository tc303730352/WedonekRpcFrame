...
namespace RpcCacheClient.Helper
{
        ...
        public class RedisDataQueue<T> : IDataQueueHelper<T>
        {
                ...
                public RedisDataQueue(string name, SyncQueueData<T> action) : this(name, action, 1000, 100)
                {
                }
                public RedisDataQueue(string name, SyncQueueData<T> action, SaveQueueFailLog<T> log) : this(name, action, 1000, 100)
                {
                        this._LogAction = log;
                }
                ...
                private void _SyncData(_QueueData data)
                {
                        ...
                        else if (!this._ExecAction(data.Data, out string error))
                        {
                                data.RetryNum += 1;
                                data.error = error;
                                if (this._MaxRetryNum < data.RetryNum)
                                {
                ...
                {
                        this._QueueList.Enqueue(new _QueueData
                        {
                                Data = data,
                                RetryNum = 0,
                ...
                {
                        this._SyncTimer.Change(dueTime, period);
                }
        }
}
