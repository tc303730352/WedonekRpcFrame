...
namespace RpcHelper
{
        ...
        public class DataQueueHelper<T> : IDataQueueHelper<T>
        {
                ...
                public DataQueueHelper() : this(1000, 100)
                {
                }
                ...
                private bool _InvokeAction(T data, out string error)
                {
                        ...
                        {
                                if (e.InnerException is ErrorException i)
                                {
                                        error = i.ToString();
                                        return i.IsEnd;
                                }
                                LogInfo log = LogSystem.CreateErrorLog(e, "队列数据处理失败！", "DataQueue");
                                log.LogContent = Tools.Json(data);
                                log.Save();
                        ...
                        if (this._Action != null)
                        {
                                if (!this._InvokeAction(data, out string error))
                                {
                                        if (this._MaxRetryNum != 0)
                                        {
                                                ...
                                                new Timer(new TimerCallback(this._RetrySync), obj, this._RetryTime, Timeout.Infinite);
                                        }
                                        else
                                        {
                ...
                private void _SaveErrorLog(T model, string error, int retryNum)
                {
                        if (this._LogAction != null)
                        {
                                ...
                                {
                                        LogInfo log = LogSystem.CreateErrorLog(e, "队列数据处理失败！", "DataQueue");
                                        log.LogContent = Tools.Json(model);
                                        log.Save();
                                }
                        }
                }
                private void _RetrySync(object state)
                {
                        ...
                        try
                        {
                                if (!this._InvokeAction(data.Data, out string error))
                                {
                                        ...
                                        {
                                                new Timer(new TimerCallback(this._RetrySync), data, this._RetryTime, Timeout.Infinite);
                                        }
                                        else
                                        {
                        ...
                        {
                                LogInfo log = LogSystem.CreateErrorLog(e, "队列数据处理失败！", "DataQueue");
                                log.LogContent = Tools.Json(data.Data);
                                log.Save();
                        }
                }
                public void Dispose()
