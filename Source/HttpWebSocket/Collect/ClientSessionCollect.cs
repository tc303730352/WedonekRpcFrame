        ...
        internal class ClientSessionCollect : IClientSessionCollect
        {
                ...
                public ClientSessionCollect(ISocketServer server)
                {
                        ...
                        if (server.Config.IsSingle)
                        {
                                ...
                                TaskManage.AddTask(new TaskHelper("同步授权", new TimeSpan(), new Action(this._SyncAccredit)));
                        }
                }

                private void _SyncAccredit()
                {
                ...
                public IClientSession[] GetSession(Guid[] sessionId)
                {
                        ...
                        {
                                if (this._SessionList.TryGetValue(a, out ISession client))
                                {
                                        return client;
                                }
                                return null;
                ...
                {
                        if (!this._Server.Config.IsSingle && session.IsAccredit && session.AccreditId == null)
                        {
                                return;
                        }
                        else if (this._AccreditKeys.TryGetValue(session.AccreditId, out Guid sessionId) && sessionId == session.SessionId)
                        {
                                this._AccreditKeys.TryRemove(session.AccreditId, out _);
                        }
                }
                ...
                {
                        if (!this._Server.Config.IsSingle && obj.AccreditId == null)
                        {
                                return;
                        }
