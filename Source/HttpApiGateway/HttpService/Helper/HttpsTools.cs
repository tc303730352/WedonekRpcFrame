...
namespace HttpService.Helper
{
        internal class HttpsTools
        {
                ...
                {
                        this.HashVal = string.IsNullOrEmpty(hash) ? ServerConfig.CertHashVal : hash;
                }
                public bool BindUri(Uri uri)
                {
                        ...
                        string ports = string.Format(":{0}", uri.Port.ToString());
                ...
                private bool _AddUrlBind(Uri uri)
                {
                        ...
                        return Tools.RunCmd(new string[] { cmd }, out string res) && res.IndexOf("错误") == -1 && res.IndexOf("失败") == -1;
                }
                ...
                private bool _SetNetshSslCert(Guid guid, string certhash, Uri uri)
                {
                        ...
                        return Tools.RunCmd(cmd, out string res) && res.IndexOf("错误") == -1 && res.IndexOf("失败") == -1;
                }
                private List<_netsh_sslcert> _GetNetshSslcert()
                {
                        if (Tools.RunCmd("netsh http show sslcert", out string res))
                        {
                                ...
                                string str = null;
                                int i = index + 2;
                                List<_netsh_sslcert> list = new List<_netsh_sslcert>();
                                do
                                {
                                        str = t[i].Trim();
