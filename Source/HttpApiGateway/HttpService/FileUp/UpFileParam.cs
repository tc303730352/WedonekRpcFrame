...
namespace HttpService
{
        ...
        /// 上传文件参数
        /// </summary>
        public class UpFileParam : System.IDisposable
        {
                private readonly MD5CryptoServiceProvider _Md5 = null;
                ...
                internal void CalculateMd5(byte[] stream, int len)
                {
                        if (this._Md5 != null)
                        {
                                this._Md5.TransformBlock(stream, 0, len, stream, 0);
                        }
                }
                internal void End(byte[] stream, int len)
                {
                        if (this._Md5 != null)
                        {
                                this._Md5.TransformFinalBlock(stream, 0, len);
