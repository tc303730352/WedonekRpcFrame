...
namespace RpcClient.Server
{
        internal class RpcServer
        {
                ...
                public RpcServer(string ip)
                {
                        ...
                        SocketConfig.AddConArg(this.ServerIp, this.ServerPort, null, Config.WebConfig.AppId);
                ...
                public bool GetAccessToken(out AccessToken token, out string error)
                {
                        ApplyAccessToken obj = new ApplyAccessToken
                        {
                                ...
                                Secret = Config.WebConfig.AppSecret
                        };
                ...
                public bool GetReduceInRank(long serverId, out ReduceInRank reduce, out string error)
                {
                ...
                internal bool SendHeartbeat(ServiceHeartbeat obj, out string error)
                {
                        return this._Send("ServiceHeartbeat", obj, out error);
                }
                ...
                public bool ServerLogin(RpcToken token, RemoteServerLogin login, out RpcConfig config, out RpcServerConfig serverConfig, out string error)
                {
                        ...
                        if (this._Send("RemoteServerLogin", obj, out RpcServerLoginRes model, out error))
                        {
                ...
                private bool _Send<T, Result>(string type, T data, out Result result, out string error) where T : class
                {
                        ...
                        RpcLogSystem.AddMsgLog(type, data, result, this);
                        return true;
                }
                ...
                public bool GetRemoteServerData(RpcToken token, long serverId, out ServerConfigInfo config, out string error)
                {
                        ...
                        else if (error.StartsWith("rpc.token."))
                        {
                                if (token.ResetToken())
                                {
                                        return this.GetRemoteServerData(token, serverId, out config, out error);
                                }
                        }
                        config = null;
                        return false;
                ...
                public bool GetRemoteServer(string sysType, long merId, int regionId, out GetServerListRes res, out string error)
                {
