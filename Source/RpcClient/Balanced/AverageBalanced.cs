        ...
        internal class AverageBalanced : IBalanced
        {
                ...
                private bool _CheckIsUsable(IRemoteConfig config, int begin, out IRemote server)
                {
                        ...
                        else
                        {
                                ...
                                if (serverId != config.FilterServerId && RemoteServerCollect.GetRemoteServer(serverId, out server) && server.IsUsable)
                                {
                                        return true;
                                }
                                else
                ...
                public bool DistributeNode(IRemoteConfig config, out IRemote server)
                {
                        if (this._ServerNum == 1)
                        {
                                ...
                                {
                                        return RemoteServerCollect.GetRemoteServer(serverId, out server) && server.IsUsable;
                                }
                                server = null;
                                return false;
                        }
