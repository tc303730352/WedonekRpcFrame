...
namespace RpcService.Route
{
        internal class TcpRoute<T> : ITcpRoute
        {
                ...
                protected virtual bool ExecAction(T param, out string error)
                {
                        error = "public.no.func";
                        return false;
                }
                protected virtual bool ExecAction(T param, string clientIp, out string error)
                {
                        return this.ExecAction(param, out error);
                }
                private IBasicRes _MsgHandle(RemoteMsg msg)
                {
                        ...
                        else if (!this.ExecAction(data, msg.ClientIp, out string error))
                        {
