...
export default {
  ...
  methods: {
    ...
    async add() {
      ...
      await taskPlanApi.Set(this.id, data)
      this.$message({
        message: '保存成功！',
        type: 'success'
      })
    ...
    async reset() {
      const data = await taskPlanApi.Get(this.id)
      const def = {
        PlanTitle: data.PlanTitle,
        PlanType: data.PlanType,
        IsEnable: data.IsEnable,
        ExecRate: data.ExecRate
      }
      if (data.PlanType === 1) {
        def.ExecTime = data.ExecTime
        this.formData = def
        return
      }
      if (data.ExecRate === 0) {
        def.ExecSpace = data.ExecSpace
      } else if (data.ExecRate === 1) {
        def.ExecSpace = data.ExecSpace
        def.week = []
        for (const i in AutoTaskSpaceWeek) {
          if (i & def.SpaceWeek === i) {
            def.week.push(i)
          }
        }
      } else if (data.ExecRate === 2) {
        def.SpaceType = data.SpaceType
        def.ExecSpace = data.ExecSpace
        if (def.SpaceType === 0) {
          def.SpaceDay = data.SpaceDay
        } else {
          def.SpeceNum = data.SpeceNum
          def.SpaceWeek = data.SpaceWeek
        }
      }
      def.DayRate = data.DayRate
      if (def.DayRate === 0) {
        def.DayTimeSpan = this.formatTimeSpan(data.DayTimeSpan)
      } else if (def.DayRate === 1) {
        def.DaySpaceNum = data.DaySpaceNum
        def.DaySpaceType = data.DaySpaceType
        def.DaySpan = [
          this.formatTimeSpan(data.DayBeginSpan),
          this.formatTimeSpan(data.DayEndSpan)
        ]
        if (def.DaySpaceType === 0) {
          def.DaySpaceNum = def.DaySpaceNum / 3600
        } else if (def.DaySpaceType === 1) {
