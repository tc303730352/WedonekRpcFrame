      ...
      <el-card style="margin-bottom:30px;">
        ...
        <el-form ref="form" :rules="rules" :model="formData" label-width="120px">
          ...
          <el-form-item label="超时时间" prop="TimeOut">
            <el-input v-model="formData.TimeOut" placeholder="超时时间">
          ...
          <el-form-item label="重试次数" prop="RetryNum">
            <el-input-number v-model="formData.RetryNum" placeholder="重试次数" />
          </el-form-item>
      ...
      <el-card v-else-if="formData.SendType === 2">
        ...
        <el-form ref="broadcast" :rules="broadcastRules" :model="broadcast" label-width="120px">
          ...
          <el-form-item v-if="rpcConfig.ToType == 0" label="服务节点类型" prop="TypeVal">
            <el-select v-model="broadcast.TypeVal" style="width: 800px;" multiple clearable placeholder="目的地节点类型">
              <el-option-group
          ...
          <el-form-item v-else label="服务节点" prop="ServerId">
            <el-select v-model="broadcast.ServerId" style="width: 800px;" multiple clearable placeholder="目的地节点">
  ...
  methods: {
    ...
    async validate() {
      ...
      if (!valid) {
        ...
        const res = await this.$refs['rpcConfig'].validate()
        if (!res) {
          return false
        }
      } else if (this.formData.SendType === 1) {
        const res = await this.$refs['http'].validate()
        if (!res) {
          return false
        }
      } else {
        const res = await this.$refs['broadcast'].validate()
        if (!res) {
          return false
        }
      }
    ...
    async save() {
      ...
      await taskItemApi.Set(this.id, add)
      this.$message({
        message: '修改成功！',
        type: 'success'
      })
    ...
    async reset() {
      const res = await taskItemApi.Get(this.id)
      this.formData = {
        ItemTitle: res.ItemTitle,
        ItemSort: res.ItemSort,
        SendType: res.SendType,
        TimeOut: res.TimeOut,
        RetryNum: res.RetryNum,
        LogRange: res.LogRange,
        FailStep: res.FailStep,
        FailNextStep: res.FailNextStep,
        NextStep: res.NextStep,
        SuccessStep: res.SuccessStep
      }
      if (res.SendType === 0) {
        const rpc = res.SendParam.RpcConfig
        this.rpcConfig = {
          SysDictate: rpc.SysDictate,
          SystemType: rpc.SystemType,
          ServerId: rpc.ServerId,
          MsgBody: null,
          ToType: 0
        }
        if (rpc.MsgBody != null) {
          this.rpcConfig.MsgBody = JSON.stringify(rpc.MsgBody, null, '\t')
        }
        if (this.isNull(rpc.SystemType)) {
          this.rpcConfig.ToType = 1
        }
        this.sendParam = rpc.RemoteSet
      } else {
        this.rpcConfig = {
          SysDictate: null,
          ToType: 0
      ...
      if (res.SendType === 1) {
        this.httpConfig = res.SendParam.HttpConfig
      } else {
        this.httpConfig = {
          RequestMethod: 'GET',
          ReqType: 0,
      ...
      if (res.SendType === 2) {
        const bro = res.SendParam.BroadcastConfig
        this.broadcast = {
          SysDictate: bro.SysDictate,
          TypeVal: bro.TypeVal,
          ServerId: bro.ServerId,
          IsOnly: bro.IsOnly,
          BroadcastType: bro.BroadcastType,
          MsgBody: null,
          ToType: 0
        }
        if (bro.MsgBody != null) {
          this.broadcast.MsgBody = JSON.stringify(bro.MsgBody, null, '\t')
        }
        if (bro.TypeVal == null && bro.TypeVal.length === 0) {
          this.broadcast.ToType = 1
        }
        this.sendParam = bro.RemoteSet
      } else {
        this.broadcast = {
          SysDictate: null,
          BroadcastType: 0,
