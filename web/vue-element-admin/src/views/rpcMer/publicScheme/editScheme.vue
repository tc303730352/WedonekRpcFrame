  ...
  <div>
    ...
    >
      ...
      >
        ...
        <el-table-column prop="toVerId" label="目的地版本号" min-width="150">
          <template slot-scope="scope">
            {{ scope.row.toVerId }}
          </template>
        </el-table-column>
...
export default {
  ...
  methods: {
      ...
      rows.forEach((c) => {
        list.push({
          ...
          children: c.SystemType.map(a => {
            ...
            let verNum
            if (ver) {
              verNum = this.formatVerNum(ver.VerNum).split('.')
              res.children = this.formatToRow(ver.ToVer, res)
              res.isEnable = true
            } else {
              verNum = a.VerNum.split('.')
              res.children = []
            }
            res.verOne = verNum[0]
            res.verTwo = verNum[1]
            res.verThree = verNum[2]
    ...
    async add() {
      ...
      await schemeApi.Set(this.id, data)
      this.$message({
        message: '添加成功！',
        type: 'success'
      })
